...
(function() {
  ...
  function startForgotPasswordCooldown(seconds = 60) {
    const forgotBtn = document.getElementById('forgotPasswordBtn');
    if (!forgotBtn) return;
    ...
    forgotPasswordCooldownTimer = setInterval(() => {
      ...
      if (remaining > 0) {
        ...
        forgotBtn.textContent = 'Resend reset link';
      }
    }, 1000);
  }
  ...
  async function handleForgotPassword() {
    ...
    if (!email || !emailRegex.test(email)) {
      showAuthAlert('Please enter a valid email address above first.', 'error');
    ...
    try {
      if (!firebaseAuth) {
        throw new Error('Authentication service is currently unavailable. Please check your network connection or cloud database configuration.');
      }
      ...
      let userMsg = 'Unable to send password reset email at this moment. Please try again.';
      ...
      // Re-enable button if no active cooldown
      if (forgotBtn && !forgotPasswordCooldownTimer) {
        forgotBtn.disabled = false;
  ...
  async function handleSignOut() {
    closeAccountMenu();
    if (confirm('Log out of your MYDAY account?')) {
      try {
        if (firebaseAuth) {
